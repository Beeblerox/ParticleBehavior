...
class PlayState extends FlxState
{
	...
	private function onBlendToggle():Void
	{
		...
		if (_isBlendOn)
		{
			_emitter.settings.blend = BlendMode.ADD;
		}
		else
		{
			_emitter.settings.blend = null;
		}
	...
	private function onLaunchModeToggle():Void
	{
		...
		if (_emitter.settings.launchMode == FlxEmitterMode.SQUARE)
		{
			_emitter.settings.launchMode = FlxEmitterMode.CIRCLE;
			
			if (_isDirectionOn)
			{
				_emitter.settings.launchAngle.set(-45, 45);
			}
		...
		{
			_emitter.settings.launchMode = FlxEmitterMode.SQUARE;
			
			if (_isDirectionOn)
			{
				_emitter.settings.velocity.set(200, -100, 200, 100);
			}
	...
	private function onDirectionToggle():Void
	{
		...
		if (_isDirectionOn)
		{
			if (_emitter.settings.launchMode == FlxEmitterMode.CIRCLE)
			{
				_emitter.settings.launchAngle.set(-45, 45);
			}
			else
		...
		{
			_emitter.settings.launchAngle.set(-180, 180);
			_emitter.settings.velocity.set( -100, -100, 100, 100);
	...
	private function onVelocityToggle():Void
	{
		...
		if (_isVelocityOn)
		{
			_emitter.settings.velocity.set(50, 40, 60, 80, -400, -600, 400, 600);
		}
		else
		{
			_emitter.settings.velocity.set( -100, -100, 100, 100);
	...
	private function onAngularVelocityToggle():Void
	{
		...
		{
			_emitter.settings.angularVelocity.set( -180, 180, -90, 90);
		}
		else
		{
			_emitter.settings.angularVelocity.set(0);
		}
	...
	private function onAngleToggle():Void
	{
		...
		if (_isAngleOn)
		{
			_emitter.settings.angle.set( -90, 90);
		}
		else
		{
			_emitter.settings.angle.set(0);
		}
	...
	private function onLifespanToggle():Void
	{
		...
		if (_isLifespanOn)
		{
			_emitter.settings.lifespan.set(0.1, 1);
		}
		else
		{
			_emitter.settings.lifespan.set(3, 3);
		}
		
		updateText("Lifespan range is " + _emitter.settings.lifespan.min + " - " + _emitter.settings.lifespan.max);
	}
	...
	private function onScaleToggle():Void
	{
		...
		if (_isScaleOn)
		{
			_emitter.settings.scale.set(1, 1, 1, 1, 4, 4, 8, 8);
		}
		else
		{
			_emitter.settings.scale.set(1);
		}
	...
	private function onAlphaToggle():Void
	{
		...
		if (_isAlphaOn)
		{
			_emitter.settings.alpha.set(1, 1, 0, 0);
		}
		else
		{
			_emitter.settings.alpha.set(1);
		}
	...
	private function onColorToggle():Void
	{
		...
		if (_isColorOn)
		{
			_emitter.settings.color.set(FlxColor.RED, FlxColor.PINK, FlxColor.BLUE, FlxColor.CYAN);
		}
		else
		{
			_emitter.settings.color.set(FlxColor.WHITE);
		}
	...
	private function onDragToggle():Void
	{
		...
		if (_isDragOn)
		{
			_emitter.settings.drag.set(200, 200, 600, 600);
		}
		else
		{
			_emitter.settings.drag.set(0);
		}
	...
	private function onAccelerationToggle():Void
	{
		...
		{
			_emitter.settings.acceleration.set(0, 0, 0, 0, 200, 200, 400, 400);
		}
		else
		{
			_emitter.settings.acceleration.set(0);
		}
	...
	private function onElasticityToggle():Void
	{
		...
		if (_isElasticityOn)
		{
			_emitter.settings.elasticity.set(1, 1, 1, 1);
		}
		else
		{
			_emitter.settings.elasticity.set(0);
		}
	...
	private function onCollisionToggle():Void
	{
		...
		if (_isCollisionOn)
		{
			_wall.revive();
			_floor.revive();
			_emitter.settings.solid = true;
		}
		else
		{
			_wall.kill();
			_floor.kill();
			_emitter.settings.solid = false;
		}
	...
	private function onGravityToggle():Void
	{
		...
		if (_isGravityOn)
		{
			_emitter.settings.acceleration.start.min.y = 800;
			_emitter.settings.acceleration.start.max.y = 1000;
			_emitter.settings.acceleration.end.min.y = 800;
			_emitter.settings.acceleration.end.max.y = 1000;
		}
		else
		{
			_emitter.settings.acceleration.set(0);
